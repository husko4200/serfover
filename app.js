    ...
    if (pageContext === 'login') {
        ...
        if (loginForm) {
            loginForm.addEventListener('submit', (e) => {
                ...
                const usernameVal = document.getElementById('username').value.toLowerCase().trim();
                const passwordVal = document.getElementById('password').value;

                const users = JSON.parse(localStorage.getItem('serfover_users')) || [];
                const foundUser = users.find(u => u.username === usernameVal && u.password === passwordVal);

                if (foundUser) {
                    localStorage.setItem('serfover_user', JSON.stringify({ ...foundUser }));
