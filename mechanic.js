...
async function loadMechanicData() {
    if (typeof API.getCachedData === 'function') {
        ...
        if (cachedData && (cachedData.reportes || cachedData.combustibles || cachedData.mantenciones)) {
            try { renderMechanicData(cachedData); } catch(e) { console.error('Cache render error', e); }
        } else {
            setMechanicCargando();
        }
    } else {
    ...
    try {
        ...
        if (data) renderMechanicData(data);
    } catch(error) {
        console.error('Error fetching fresh data', error);
    }
}

function setMechanicCargando() {
...
function renderMechanicData(data) {
    if (!data) return;
    try {
        ...
        console.error('Error renderizando datos del mecánico', error);
    }
}
